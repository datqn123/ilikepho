...
- Sửa `AdminAuthInterceptor`: cho qua ngay khi request attribute `adminSession` đã được remember-me đặt (cookie phiên mới chỉ nằm trên response, request không tự thấy).
...
| FR-05 | `RememberMeInterceptor` (chạy **trước** `AdminAuthInterceptor`, trừ `/admin/logout`): nếu request **đã có phiên hợp lệ** → bỏ qua. Nếu **chưa có phiên** nhưng có cookie `ADMIN_REMEMBER` hợp lệ & chưa hết hạn → tạo phiên mới qua `createSession`, set cookie `ADMIN_SESSION` mới, **rotate** remember token (xoá token cũ, tạo token mới, set cookie mới). | Thiếu cookie remember → return ngay, không truy vấn DB. Khi tạo phiên mới → set request attribute `adminSession`. |
| FR-06 | Nếu remember token không tồn tại / hết hạn → không tạo phiên, để `AdminAuthInterceptor` xử lý redirect login như hiện tại. | |
| FR-07 | POST `/admin/logout`: khoá phiên hiện tại + xoá remember token (theo hash cookie `ADMIN_REMEMBER` trong request — bao phủ cả khi phiên đã chết; và theo `adminId` của phiên vừa khoá) + gửi cookie hết hạn cho cả `ADMIN_SESSION` lẫn `ADMIN_REMEMBER` → redirect login. | |
...
- **Xử lý thời gian**: kiểm tra `expiresAt` của remember token là phép tính khoảng thời gian — phải qua kiểu nhận biết múi giờ (`ZonedDateTime`/`Instant`) với ZoneId chung `Asia/Ho_Chi_Minh` (admin-build mục 6), tái sử dụng mẫu `AdminSessionService.isExpired`; không so `LocalDateTime` trần.
...
- **Luồng dữ liệu / luồng xử lý**:
  ...
  - **Request chưa có phiên, có remember cookie hợp lệ**: `RememberMeInterceptor.preHandle` → hash token từ cookie → tra DB (còn hạn) → `AdminSessionService.createSession(adminId, rawSessionIdMoi)` → set cookie `ADMIN_SESSION` → rotate remember token (xoá cũ + tạo mới + set cookie mới) → set request attribute `adminSession` (phiên vừa tạo) → `return true` → `AdminAuthInterceptor` (chạy sau) thấy attribute đã có → cho qua ngay. Lý do: cookie mới chỉ nằm trên response, request vẫn mang cookie cũ nên auth đọc lại cookie sẽ không thấy phiên mới.
  - **Logout**: controller gọi `AdminAuthService.logout(rawSessionId, rawRememberToken)` (khoá phiên) + xoá remember token theo hash cookie `ADMIN_REMEMBER` (bao phủ cả khi phiên đã chết, không có `adminId`) và theo `adminId` của phiên vừa khoá → gửi cookie hết hạn cho cả 2 cookie.
- **Các quyết định thiết kế**:
  ...
  - **Nối phiên qua request attribute `adminSession`**: remember-me tạo phiên thì tự set attribute; `AdminAuthInterceptor` được sửa để cho qua ngay khi attribute đã tồn tại (cookie phiên mới không thể "chèn ngược" vào request). Remember-me **chỉ** set attribute khi nó tạo phiên mới — nếu set cả khi phiên đã hợp lệ, auth sẽ bỏ qua sliding expiration và kiểm tra hết hạn. Nhánh cho qua sớm của auth vẫn phải set attribute `csrfToken` từ phiên trong `adminSession` (thay đổi S-04 Task 05 chỉ nằm trên đường xác thực thường) — nếu không, trang render ngay sau khi nối phiên sẽ có form logout thiếu token và POST logout bị 403.
  - **Early return khi không có cookie `ADMIN_REMEMBER`**: interceptor không truy vấn DB trên request thường (chỉ đọc cookie), tránh thêm query cho toàn hệ thống.
...
- **S-03**: Sửa `SessionCookieService`: thêm `createRememberMeCookie(String token)` và `createExpiredRememberMeCookie()` (MaxAge inject từ property `admin.remember-me.max-age-days` vào `SessionCookieService`, giống cách inject cờ `secure` hiện có).
- **S-04**: Sửa `AdminAuthService`: `login` nhận thêm `boolean rememberMe`, tạo remember token khi cần, trả kết quả gồm session id + remember token (nullable); `logout(rawSessionId, rawRememberToken)` xoá remember token theo hash cookie `ADMIN_REMEMBER` (nếu request có) và theo `adminId` khi khoá được phiên.
...
- **S-06**: Tạo `RememberMeInterceptor` trong `admin.interceptor` (nối lại phiên + rotate; khi tạo phiên mới → set request attribute `adminSession`); sửa `AdminAuthInterceptor`: đầu `preHandle`, nếu attribute `adminSession` đã tồn tại → set luôn attribute `csrfToken` từ phiên đó rồi cho qua ngay.
...
| `src/main/java/com/restaurant/ilikepho/admin/interceptor/AdminAuthInterceptor.java` | admin | `admin.interceptor` | sửa | Cho qua ngay khi request attribute `adminSession` đã tồn tại (remember-me vừa tạo phiên) (FR-05 / S-06). |
...
- **Rủi ro**: set attribute `adminSession` cho cả request phiên đã hợp lệ khiến auth bỏ qua sliding expiration/kiểm tra hết hạn. Giảm thiểu: remember-me chỉ set attribute khi nó tự tạo phiên mới (ghi rõ ở S-06); request có cookie remember nhưng phiên hợp lệ thì auth chạy bình thường.
- **Rủi ro (ghi nhận sau code review)**: chính sách "1 admin = 1 token sống" chỉ được service bảo đảm (`createToken` xoá trước khi tạo), chưa có unique constraint ở DB trên `admin_remember_me.admin_id` — hai login/rotate chạy đồng thời (race) có thể tạm giữ 2 token sống của cùng một admin. Chấp nhận trong phạm vi task này: cả hai token đều là thông tin đăng nhập hợp lệ của chính admin đó (không leo quyền), logout xoá sạch theo `adminId`. **Xử lý triệt để đã chuyển vào Task 07** — bọc `@Transactional` cho `AdminAuthService.login/logout` để tuần tự hoá tạo token nhờ pessimistic lock sẵn có trên dòng admin (không cần unique index, không biến race thành 500).
...
- [x] **DoD-05**: Request chưa có phiên + remember token hợp lệ → `RememberMeInterceptor` tạo phiên ACTIVE mới, set cookie `ADMIN_SESSION` và request đi qua được toàn bộ chuỗi interceptor (auth không redirect login); phiên cũ của admin bị khoá (test pass). *(RememberMeInterceptorTest: phienChet/phienHetHan/matCookiePhien + remember hợp lệ → createSession + set cookie + set attribute; AdminAuthInterceptorTest.phienDoRememberMeNoiLai → auth cho qua không redirect; phiên cũ bị khoá bởi AdminSessionService.createSession — lockAllActiveByAdminId có test từ Task 04)*
- [x] **DoD-06**: Remember token được rotate sau mỗi lần nối lại phiên: token cũ vô hiệu, token mới còn hạn (test pass). *(AdminRememberMeServiceTest.rotate_xoaTokenCuVaTaoTokenMoi; kèm guard không xoá token của admin khác)*
- [x] **DoD-07**: Remember token không tồn tại hoặc hết hạn → không tạo phiên, không ném lỗi (test pass). *(findValidByTokenHash_tokenHetHan_traVeRong; RememberMeInterceptorTest.preHandle_rememberKhongHopLe_khongTaoPhienDeAuthXuLy)*
...
- Cleanup định kỳ dòng remember token hết hạn. *(đã chuyển vào Task 07 — FR-09)*
- Dọn cookie `ADMIN_REMEMBER` cũ trên trình duyệt khi đăng nhập không nhớ. *(phát sinh sau code review, đã chuyển vào Task 07 — FR-06)*
- Remember-me cho hệ user.
- Giao diện/tuỳ chọn thời hạn nhớ trên UI.
- Thay đổi chính sách phiên hiện có ngoài việc bổ sung nối lại phiên.
- Test chuỗi đầy đủ 3 interceptor bằng MockMvc. *(phát sinh sau code review, đã chuyển vào Task 07 — FR-10)*
